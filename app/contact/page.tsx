...
const fadeInUp: Variants = {
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.6, ease: [0.16, 1, 0.3, 1], type: 'tween' }
  }
}
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const timer = setTimeout(() => { if (mounted) setStatus('') }, 5000)
  ...
  return (
    ...
    >
      ...
      {/* Navigation Bar */}
      <motion.div className="max-w-6xl mx-auto py-4 relative z-10" variants={fadeInUp}>
        <div className="flex items-center justify-between">
  <Link href="/about" className="flex items-center text-gray-400 hover:text-white transition-colors group">
    <ArrowLeft size={20} className="mr-2 group-hover:-translate-x-1 transition-transform" />
    <span>{tr.backToPortfolio}</span>
  </Link>
  ...
  <Link href="/" className="flex items-center text-gray-400 hover:text-white transition-colors group">
    ...
    <span>{tr.home}</span>
      ...
      >
        <motion.div className="text-center mb-8 sm:mb-16 px-2" variants={fadeInUp}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 lg:gap-12">
          {/* Contact Info */}
          <motion.div className="h-full flex flex-col" variants={fadeInUp}>
            <div className="bg-white/5 backdrop-blur-sm p-6 sm:p-8 rounded-2xl border border-white/5 shadow-2xl flex-1 flex flex-col justify-center">
              ...
              <div className="space-y-4 sm:space-y-6 pt-6 sm:pt-8">
                ...
                >
                  ...
                  <div>
                    <p className="text-white font-medium">{tr.emailLabel}</p>
                    <p className="text-gray-300 group-hover:text-blue-400 transition-colors">blkfatine2@gmail.com</p>
                  </div>
                </div>

                <a href="tel:+212635291627" className="flex items-center space-x-4 p-3 sm:p-4 rounded-xl hover:bg-white/5 transition-colors group">
                  ...
                  <div>
                    <p className="text-white font-medium">{lang === 'en' ? 'Phone' : 'Téléphone'}</p>
                    <p className="text-gray-300 group-hover:text-green-400 transition-colors">+212 6 35 29 16 27</p>
                  </div>
                </a>

                <div className="flex items-center space-x-4 p-3 sm:p-4 rounded-xl hover:bg-white/5 transition-colors group cursor-pointer">
                  ...
                  <div>
                    <p className="text-white font-medium">{lang === 'en' ? 'Location' : 'Localisation'}</p>
                    <p className="text-gray-300 group-hover:text-purple-400 transition-colors">{tr.location}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-auto pt-10 border-t border-white/10">
                <h3 className="text-white font-medium mb-4 text-center">{tr.findMeOn}</h3>
                <div className="flex space-x-4 justify-center">
                  <a href="https://linkedin.com/in/fatine-belkhammar-791aa1239" target="_blank" rel="noopener noreferrer"
                    className="p-3 bg-white/5 rounded-full text-gray-300 hover:bg-blue-600 hover:text-white transition-colors" aria-label="LinkedIn">
                    <Linkedin size={18} />
                  </a>
                  <a href="https://github.com/Blkfatine" target="_blank" rel="noopener noreferrer"
                    className="p-3 bg-white/5 rounded-full text-gray-300 hover:bg-gray-700 hover:text-white transition-colors" aria-label="GitHub">
                    <Github size={18} />
                  </a>
                </div>
              </div>
          ...
          <motion.div className="relative" variants={fadeInUp}>
            <div id="message-section" className="bg-white/5 backdrop-blur-sm p-5 sm:p-6 md:p-8 rounded-2xl border border-white/5 shadow-2xl transition-all duration-300">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={tr.namePlaceholder} required disabled={isSubmitting}
                    />
                    {formData.name && (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={tr.emailPlaceholder} required disabled={isSubmitting}
                      pattern="[^@\s]+@[^@\s]+\.[^@\s]+"
                    />
                    {formData.email && (
                  ...
                  {!isValidEmail && formData.email && (
                    <p className="text-red-400 text-sm mt-1">
                      {lang === 'en' ? 'Please enter a valid email address' : 'Veuillez entrer une adresse e-mail valide'}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      placeholder={tr.messagePlaceholder} required disabled={isSubmitting} maxLength={1000}
                    />
                ...
                <motion.button
                  type="submit" disabled={isSubmitting || !isValidEmail}
              ...
              <AnimatePresence>
                {status === 'success' && (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-green-100 font-medium">{tr.successTitle}</p>
                      <p className="text-green-300/80 text-sm mt-0.5">{tr.successText}</p>
                    </div>
                  </motion.div>
                )}
                {status === 'error' && (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-red-100 font-medium">{tr.errorTitle}</p>
                      <p className="text-red-300/80 text-sm mt-0.5">{tr.errorText}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      >
        <div className="max-w-6xl mx-auto px-4">
          <p>© {new Date().getFullYear()} Fatine Belkhammar. {tr.footer}</p>
          <p className="mt-2 text-xs text-gray-600">{tr.builtWith}</p>
        </div>
      </motion.footer>
    </motion.main>
  )
